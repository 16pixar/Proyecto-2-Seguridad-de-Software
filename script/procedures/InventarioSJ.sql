-- ============================================================
-- STORED PROCEDURES DE INVENTARIO - SAN JOSÉ
-- ============================================================
-- Procedimientos para gestión de inventario en sucursal San José
...
BEGIN
    ...
    LEFT JOIN Warehouse.StockItemHoldings_SJ wh
...
BEGIN
    ...
    -- Holdings (inventario de San José)
    SELECT
        wh.QuantityOnHand,
        wh.BinLocation
    FROM Warehouse.StockItemHoldings_SJ wh
...
BEGIN
    ...
    BEGIN TRANSACTION;
        ...
        -- Insertar en Holdings de San José
...
BEGIN
    ...
    BEGIN TRANSACTION;
        ...
        -- Actualizar cantidad en Holdings de San José
...
BEGIN
    ...
    BEGIN TRANSACTION;
        ...
        -- Eliminar holdings de San José
        DELETE FROM Warehouse.StockItemHoldings_SJ
        WHERE StockItemID = @StockItemID;
        
        -- Eliminar transacciones de San José
        DELETE FROM Warehouse.StockItemTransactions_SJ
        WHERE StockItemID = @StockItemID;
...
BEGIN
    ...
    -- Contar facturas (ventas) de San José
    SELECT @InvoiceCount = COUNT(*)
    FROM Sales.InvoiceLines_SJ 
    WHERE StockItemID = @StockItemID;
    
    -- Contar órdenes de compra de San José
    SELECT @PurchaseOrderCount = COUNT(*)
    FROM Purchasing.PurchaseOrderLines_SJ 
    WHERE StockItemID = @StockItemID;
...
PRINT '✅ Stored Procedures de Inventario creados para SAN JOSÉ';
PRINT '';
...
PRINT '';
PRINT '⚠️  TABLAS ESPECÍFICAS DE SAN JOSÉ:';
PRINT '    - StockItemHoldings_SJ';
PRINT '    - StockItemTransactions_SJ';
PRINT '    - InvoiceLines_SJ';
PRINT '    - PurchaseOrderLines_SJ';
GO
